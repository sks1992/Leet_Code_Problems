...
fun main() {
    val candies:IntArray = intArrayOf(12, 1, 12)
    val extraCandies = 10
...
fun kidsWithCandies(candies: IntArray, extraCandies: Int): List<Boolean> {
    val answer: MutableList<Boolean> = ArrayList()
    var maxCandies = candies[0]
    //find the max of candies arraylist
    //here we don't check for case when array is empty because in question there is given min array is size 2
    for (i in 1..<candies.size) {
        maxCandies = max(maxCandies.toDouble(), candies[i].toDouble()).toInt()
    }
...
class Solution {
    fun kidsWithCandies(candies: IntArray, extraCandies: Int): List<Boolean> {
        ...
        var max = candies[0]
        for (i in 1..<candies.size) {
            max = max(max.toDouble(), candies[i].toDouble()).toInt()
        }
        for (i in candies.indices) {
